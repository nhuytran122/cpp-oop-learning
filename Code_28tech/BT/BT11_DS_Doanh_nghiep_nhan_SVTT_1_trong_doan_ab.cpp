...
Tiep theo la 1 dong ghi so truy van q. Moi truy van la 2 so nguyen a, b 
viet tren 1 dong trong do a < b va du lieu dam bao luon co it nhat 1 doanh nghiep 
nhan so luong SV trong doan [a, b]

-- Output:
Ghi ra DS da loc trong doan [a, b] va duoc sap xep theo so luong giam dan, 
moi thong tin ghi tren 1 dong. Trong truong hop co cung so luong thi sap xep 
theo ma doanh nghiep (thu tu tu dien tang dan)
...
int main(){
    ...
    for(DoanhNghiep i : a){
    	if(i.getSoluong() >= x && i.getSoluong() <= y){
    		i.xuat();
		}
    }
}
