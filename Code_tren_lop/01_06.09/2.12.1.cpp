...
void Nhap(int b[][40], int &h, int &c){
	cout << "So hang: "; cin >> h;
	cout << "So cot: "; cin >> c;
	for(int i = 0; i < h; i++){
		for(int j = 0; j < c; j++){
...
void XuatMang(int b[][40], int h, int c){
    for (int i = 0; i < h; i++){
        for (int j = 0; j < c; j++){
        	cout << b[i][j] << " ";	
		}
...
void Plus(int a[][40], int b[][40], int c[][40], int h1, int c1){
	for (int i = 0; i<h1; i++){
        for (int j = 0; j<c1; j++){
        	c[i][j] = a[i][j] + b[i][j];	
		}
...
int main(){
	...
	Plus(mt1, mt2, mt, h1, c1);
	cout << "KET QUA PHEP CONG 2 MANG 2 CHIEU: \n";
	XuatMang(mt, h1, c1);
	delete a, b, c;
	return 0;
}
